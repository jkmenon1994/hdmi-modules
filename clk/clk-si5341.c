		...
		unsigned long parent_rate)
{
	...
	dev_dbg(&synth->data->i2c_client->dev,
			"%s(%u): n=0x%llx d=0x%x %s, rate : %lu\n", __func__,
				synth->index, n_num, n_den,
				is_integer ? "int" : "frac", rate);
		...
		unsigned long parent_rate)
{
	...
	dev_dbg(&output->data->i2c_client->dev,"%s: parent_rate: %lu, rate : %lu,  r_div: %lx \n", __func__, parent_rate, rate,  r_div);
	if (r_div <= 1)
		r_div = 0;
	...
	dev_dbg(&output->data->i2c_client->dev,"%s: setting rate: %lu, at index %u , r_div: %lx \n",
			__func__, rate, output->index, r_div); 
	...
	dev_dbg(&output->data->i2c_client->dev,"%s: r[0]: %x, r[1] : %x, r[2]: %x \n", __func__, r[0],
					r[1], r[2]);
	...
	int res;
        ...
        res = regmap_read(output->data->regmap, 0x10B , &val);
        if (res < 0)
                printk(KERN_ERR "error reading regmap \n");
        else
                printk(KERN_ERR " reg: 0x10B, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x110 , &val);
         printk(KERN_ERR " reg: 0x110, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x115 , &val);
         printk(KERN_ERR " reg: 0x115, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x11A , &val);
         printk(KERN_ERR " reg: 0x11A, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x11F , &val);
         printk(KERN_ERR " reg: 0x11F, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x124 , &val);
         printk(KERN_ERR " reg: 0x124, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x129 , &val);
         printk(KERN_ERR " reg: 0x129, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x12E , &val);
         printk(KERN_ERR " reg: 0x12E, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x133 , &val);
         printk(KERN_ERR " reg: 0x133, val : %x \n", val);
         val = 0;
         regmap_read(output->data->regmap, 0x13D , &val);
         printk(KERN_ERR " reg: 0x13D, val : %x \n", val);
         val = 0;
...
static int si5341_finalize_defaults(struct clk_si5341 *data)
{
	...
	res = regmap_write(data->regmap, 0x514, 0x01);
		if (res < 0)
			return res;
...
static int si5341_clk_select_active_input(struct clk_si5341 *data)
{
	...
	if (res < 0)
 	...
 	dev_info(&data->i2c_client->dev,"2. %s \n", __func__);
		...
		const struct i2c_device_id *id)
{
	...
	dev_info(&data->i2c_client->dev,
		...
		dev_info(&data->i2c_client->dev,"si5341 read settings\n");
		err = si5341_send_preamble(data);
		if (err < 0)
			return err;

		dev_info(&data->i2c_client->dev,"2. initialization required \n");
		/*
		...
		dev_info(&data->i2c_client->dev,"3. initialization required \n");
		...
		dev_info(&data->i2c_client->dev,"4. initialization required \n");
	}
	...
	if (err < 0)
		...
		dev_info(&data->i2c_client->dev,"5. initialization required \n");
#if 0
	if (initialization_required) {
	...
	data->hw.init = &init;
		...
		dev_info(&data->i2c_client->dev,"6. initialization required \n");
	...
	if (err) {
		...
		dev_info(&data->i2c_client->dev,"7. initialization required \n");
	init.num_parents = 1;
	...
	for (i = 0; i < data->num_synth; ++i) {
		...
		dev_info(&data->i2c_client->dev,"8. initialization required \n");
